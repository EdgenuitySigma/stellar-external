...
#include "imgui.h"
#include "imgui_internal.h" // For internal ImGui functions
...
#include <string>
    ...
    bool Keybind(const char* label, const char* description, int* key) {
        ...
        ImGui::ItemSize(ImRect(rect.Min, rect.Max));
        ...
        bool hovered = ImGui::ItemHoverable(rect, id, NULL);

        static std::map<ImGuiID, ImVec4> anim;
        auto it_anim = anim.find(id);

        if (it_anim == anim.end()) {
            anim.insert({ id, ImVec4(1.0f, 1.0f, 1.0f, 1.0f) });
            it_anim = anim.find(id);
        }

        it_anim->second = ImLerp(it_anim->second, g.ActiveId == id ? ImVec4(0.8f, 0.8f, 0.8f, 1.0f) : hovered ? ImVec4(0.9f, 0.9f, 0.9f, 1.0f) : ImVec4(1.0f, 1.0f, 1.0f, 1.0f), ImGui::GetIO().DeltaTime * 6.f);

        window->DrawList->AddRectFilled(clickable.Min, clickable.Max, ImGui::GetColorU32(it_anim->second), 2.0f);

        RenderTextColor(ImGui::GetFont(), rect.Min + ImVec2(10, 0), rect.Max, ImGui::GetColorU32(ImVec4(1.0f, 1.0f, 1.0f, 1.0f)), description, ImVec2(0.0, 0.8));

        ImGui::GetWindowDrawList()->AddRectFilled(rect.Min + ImVec2(width - 80, 0), rect.Max, ImGui::GetColorU32(ImVec4(0.2f, 0.2f, 0.2f, 1.0f)), 2.0f);
        ImGui::GetWindowDrawList()->AddRectFilledMultiColor(rect.Min, rect.Max - ImVec2(80, 0), ImGui::GetColorU32(ImVec4(0.2f, 0.2f, 0.2f, 0.f)), ImGui::GetColorU32(ImVec4(0.2f, 0.2f, 0.2f, 1.0f)), ImGui::GetColorU32(ImVec4(0.2f, 0.2f, 0.2f, 1.0f)), ImGui::GetColorU32(ImVec4(0.2f, 0.2f, 0.2f, 0.f)), 2.0f);

        window->DrawList->AddRectFilled(clickable.Min, clickable.Max, ImGui::GetColorU32(ImVec4(0.3f, 0.3f, 0.3f, 1.0f)), 2.0f);

        RenderTextColor(ImGui::GetFont(), rect.Min + ImVec2(10, 0), rect.Max, ImGui::GetColorU32(ImVec4(1.0f, 1.0f, 1.0f, 1.0f)), label, ImVec2(0.0, 0.2));

        RenderTextColor(ImGui::GetFont(), clickable.Min, clickable.Max, ImGui::GetColorU32(ImVec4(1.0f, 1.0f, 1.0f, 1.0f)), buf_display, ImVec2(0.5f, 0.5f));
   ...
   //     GetWindowDrawList()->AddRect(parent_window->DC.CursorPos, parent_window->DC.CursorPos + size_arg, GetColorU32(c::accent), 0);
    ...
    void Separator_line()
    {
        GetWindowDrawList()->AddRectFilled(GetCursorScreenPos(), GetCursorScreenPos() + ImVec2(GetContentRegionMax().x - GetStyle().WindowPadding.x, 1), GetColorU32(c::elements::stroke));
        Spacing();
    }
